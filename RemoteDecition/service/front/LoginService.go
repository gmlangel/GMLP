...
func (ls *LoginService)sign(ctx iris.Context){
	...
	res,err := ls.SqlPro.Query(fmt.Sprintf("SELECT * FROM `RDUser` WHERE `signName` = '%s';",loginName))
	//PWDFaild = "3";//密码错误
	resStruct := models.CurrentResponse{Code:InvalideLoginName};
	if err == nil{
		if len(res) > 0{
			if pwd,gcontains:=res[0]["signPWD"] ;gcontains==true && string(pwd) == loginPWD{
				...
				//返回成功登录结果
				resLoginStruct := models.CurrentResponse{};
